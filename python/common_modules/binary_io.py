...
def read_data_direct(inputfilename,nx,ny,nz,dtypein,undef_in=default_undef_val,undef_out=default_undef_val,seq_acces=False,order='c'):
...
#f32 for little endian data with single precission ... and so on.
   ...
   if  os.path.exists(inputfilename) :
     ...
     if ( seq_acces ) :
       ...
       if order == 'c' or order == 'C'   :  #C order
           field=np.reshape(field,(nz,nx,ny))  #Get a 3D-array
           field=field.transpose(1,2,0)
       elif order == 'f' or order == 'F' :  #Fortran order
           field=np.reshape(field,(nz,ny,nx))  #Get a 3D-array
...
def write_data_direct_woundef(inputfilename,field,dtypein,order='c'):
